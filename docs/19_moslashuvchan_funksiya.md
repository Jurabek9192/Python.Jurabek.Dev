...
print(yigindi())                    # 0 — argumentsiz ham ishlaydi
```

`*args` so'zining o'zi shart emas — muhim bo'lgani `*` belgisi (`*sonlar` ham ishlaydi).

## **kwargs — nomlangan argumentlar

```python
def profil(**kwargs):
    print(kwargs)           # dictionary ko'rinishida keladi
...
## Barchasini birga — to'g'ri tartib

```python
def malumot(ism, *args, standart_param="qiymat", **kwargs):
    # tartib: oddiy parametr -> *args -> standart qiymatli -> **kwargs
    print(f"Ism: {ism}")
    print(f"Qo'shimcha: {args}")
    print(f"Standart: {standart_param}")
    print(f"Kalit-qiymat: {kwargs}")

malumot("Sardor", 25, "Toshkent", kasb="dasturchi")
```

## List/dict'ni funksiyaga "yoyib" uzatish (unpacking chaqirishda)
...
print(qoshish(**parametrlar))    # qoshish(a=10, b=20, c=30) ga teng
```

## *args ni ro'yxat sifatida ishlatish
...
print(eng_kattasi(3, 7, 2, 9, 4))     # 9 — *args ichida oddiy for bilan aylanish mumkin
```

## Bir nechta funksiyani zanjirlab chaqirish (*args bilan)

```python
def statistika(*sonlar):
    return {
        "yigindi": sum(sonlar),
        "ortacha": sum(sonlar) / len(sonlar) if sonlar else 0,
        "eng_katta": max(sonlar) if sonlar else None,
        "eng_kichik": min(sonlar) if sonlar else None
    }
...
8. `*args` yordamida kelgan sonlar ichidan eng kattasini toping (`max()`siz).
